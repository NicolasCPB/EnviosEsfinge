...
def obterChavePacote():
    ...
    def pegarChave(anoMes):
        try:
            url = urlBase + '/v5/registroscontabeis/municipais/iniciarEnvio'
            params = {
                'anoMes': anoMes
            }
...
def enviaMultiplosJsons(quantidadeArquivos):
    ...
    while count < int(quantidadeArquivos) + 1:
        ...
        try:
            url = urlBase + '/v5/registroscontabeis/municipais/enviarParcial'
            ...
            response_text = e.response.text if e.response else 'Sem resposta'
            msg = f'Erro ao enviar parcial de número: {count}, Status Code: {status_code}'
            montarLogEnvioRemessa(msg, response_text)
            if status_code == 401:
...
if (input("Deseja chamar a finaliza? [1] Sim | [2] Não: ") == "1"):
        try:
            url = urlBase + '/v5/registroscontabeis/municipais/finalizarEnvio'
