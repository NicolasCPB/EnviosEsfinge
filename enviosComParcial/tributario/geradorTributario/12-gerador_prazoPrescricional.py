...
def generate_objects():
    ...
    for i in range(num_objects):
        data =  {
    "anoLancamentoCreditoTributarioOuDividaAtiva": 2024,
    "codigoLancamentoCreditoTributarioOuNumeroInscricaoDividaAtiva": f"Teste.{i+1}",
    "dataAtualizacaoPrazoPrescricional": "2024-12-01",
    "dataInicioPrazoPrescricional": "2024-12-01"
      }
       
        inicioPrazoPrescricional.append(data)
    
    
    file_path = os.path.join(directory, "inicioPrazoPrescricional.json")
    ...
    with open(file_path, 'w') as f:
        json.dump({"inicioPrazoPrescricional": inicioPrazoPrescricional}, f, indent=4)
    
    print("Arquivo salvo com sucesso!")

root = tk.Tk()
root.title("Gerador prescricional")
